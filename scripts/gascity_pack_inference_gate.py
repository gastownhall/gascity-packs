...
GASTOWN_BUILD_WORKFLOW_CONTRACTS = {
    ...
    "mol-refinery-patrol": (
        'if [[ ! "$GC_AGENT" =~ ^[A-Za-z0-9][A-Za-z0-9_.-]*(/[A-Za-z0-9][A-Za-z0-9_.-]*)*$ ]]; then',
        "REFINERY_BD_SCOPE=()",
        'jq -ecs --arg agent "$GC_AGENT"',
        'error("expected every array entry to be an object")',
        '.assignee == $agent',
        'type == "string" and test("[^[:space:]]")',
        'gc bd list "${REFINERY_BD_SCOPE[@]}"',
        "--has-metadata-key=branch --limit=1 --json",
        "printf -v REFINERY_QUERY",
        '\'status=open AND assignee="%s" AND type!=epic\' "$GC_AGENT"',
        'gc bd query "${REFINERY_BD_SCOPE[@]}"',
        '"$REFINERY_QUERY" --limit=21 --json',
        '[ "$REFINERY_ROW_COUNT" -ge 21 ]',
        "reached its safety bound",
