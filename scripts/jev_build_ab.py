...
def stop_disposable_dolt(city):
    ...
    expected = 'dolt sql-server --config ' + str(config)
...
def run(args, arm, out):
    ...
    workspace = gate.write_gate_workspace(out/'workspace', pack_source=pack.source,
        roles_source=pack.roles_source, city_name='jev-ab-'+out.name, rig_name='fixture')
    ...
    env['HOME'] = str(workspace.gc_home)
    # This city owns a NEW disposable local database; permit its initial migrations.
    # Never set this in the user's global shell or against an existing city.
    ...
    save(out/'source-hashes.json', {str(p.relative_to(ROOT)): hashlib.sha256(p.read_bytes()).hexdigest()
        for p in [Path(__file__), ROOT/'scripts/gascity_pack_inference_gate.py',
