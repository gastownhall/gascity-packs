...
def cli_assess(state, model, directory, timeout=120):
    prompt = ('You are the Gas City test-evidence reviewer. Evaluate the supplied '
              ...
              'coverage. Do not use tools.\n' + json.dumps({'questions': jev.questions(state), 'state': state}))
...
def run(case, arm, model, directory, jev_model, threshold):
    ...
    try:
        ...
        if arm == 'baseline':
            ...
            if uncertain:
                # A dispatched fallback with missing telemetry is unknown, not zero.
                report['llm_usage'] = None
        ...
        # Preserve billed/processed work even when parsing or validation fails.
        telemetry = directory / ('fallback/telemetry.json' if (directory/'fallback').exists() else 'telemetry.json')
        if telemetry.exists():
            report['llm_usage'] = json.loads(telemetry.read_text())
...
def summarize(root):
    ...
    for arm in ('baseline', 'jev'):
        ...
        rows.append({'arm': arm, 'attempts': len(all_rows), 'completed': len(completed),
                     ...
                     'accuracy_all_attempts': sum(r['correct'] for r in completed)/len(all_rows) if all_rows else None,
                     'elapsed_seconds_all_attempts': sum(r['elapsed_seconds'] for r in all_rows),
                     'llm_usage_unknown_attempts': sum(r['llm_usage'] is None for r in all_rows),
                     'known_llm_tokens_all_attempts': sum(r['llm_usage']['totals']['totalTokens'] for r in all_rows if r['llm_usage'] is not None),
...
def main():
    ...
    (args.out / 'jev_evidence.py').write_bytes(HELPER.read_bytes())
    (args.out / 'jev_ab.py').write_bytes(Path(__file__).read_bytes())
    schedule = []
