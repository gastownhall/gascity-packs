# Gas City Jev Build Pack (`gascity-jev`, imported as `gc`)

This experimental alternative to [gascity](../gascity) preserves the full
build workflow and adds five bounded Jev integrations: issue/PR kind, duplicate
candidate ordering, test-evidence assessment, finding categories/pair matching,
and failure investigation routing. All five default to `auto`: they use
`TYPESAFE_API_KEY` in the worker environment when available and fall back to
ordinary handling without it. Confidence/error fallbacks and review gates remain.
...
The historical [all-enabled experiment](../specs/experiments/jev-all-enabled/RESULTS.md)
measured 33.8% fewer Claude tokens and 37.1% less time on a constructed workload,
with one additional wrong categorical answer (107/108 versus 106/108). Both
full-runtime attempts failed before Jev execution. These results predate this
pack split and do not establish an end-to-end speedup or quality equivalence
between the two packs. Keeping all five enabled preserves the experiment
configuration, including its measured tradeoffs.

The inherited workflow gathers requirements, writes and reviews a plan,
decomposes tasks, implements in parallel agent sessions, reviews the result,
and optionally publishes. It ships three things:
...
- **The `build-base` contract** — a copy of the virtual stage sequence from
  the base pack. The existing methodology packs continue to extend the sibling
  `gascity` implementation.
...
latest-release URL will not install this new pack until it has been published.

1. Import formulas and the claim command. From the city directory:

   ```sh
   gc import add --name gc /absolute/path/to/gascity-packs/gascity-jev
   ```
   ...
   command. Keep `gascity-jev` imported as `gc` at city scope and use its own roles.
   When switching an existing city, replace its base-pack source and matching
   roles source instead of adding a second import alias.
...
providerless rig role agents under `gascity-jev/roles`. Standalone use requires
the top-level `gc` import for formulas, mayor skill, and `gc gc claim`, and
`gascity-jev/roles` on each target rig for `gc.*` role agents.
...
[rigs.imports.gc]
source = "/path/to/gascity-packs/gascity-jev/roles"
...
Method, and garrytan/gstack. Those packs import the sibling `gascity` pack as
`gc` internally. Using this Jev variant beneath them would require a separate
composition change and validation; importing them alongside `gascity-jev` does
not select this variant automatically.
...
This variant keeps its inherited requirements and Jev additions beside the formula implementation:
...
For an internal ablation of this variant, pass all three: `--var jev_mode=off
--var jev_findings_mode=off --var jev_failure_mode=off`. A configured `auto`
mode still uses ordinary handling for uncertain or failed decisions. This
mode-off ablation is distinct from comparing against the restored `gascity` pack.
