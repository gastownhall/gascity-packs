Finalize the `jev-build` workflow (build-basic with a Jev review gate).
...
The jev-build implementation result may live in a source anchor/worktree. A
...
`workflow: jev-build`. The top-level YAML shape must be:

- `schema: gc.build.final-report.v1`
- `workflow: {id: <workflow-root-id>, formula: <root gc.formula_name, normally jev-build>}`
- `methodology: {pack: gascity-jev, name: jev-build}`
- `producer: {formula: jev-build, stage: finalize, attempt: <positive integer>}`
...
- methodology: jev-build (build-basic with a Jev review gate)
- requirements, plan, decomposition, implementation, and review artifact paths
- implementation convoy id
- review lanes that ran and lanes the Jev gate skipped (`jev.summary` on the gate bead, also in the review report)
