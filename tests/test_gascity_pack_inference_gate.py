...
def test_validate_gastown_orchestration_contract_rejects_split_terminal_action(tmp_path) -> None:
    pack = write_gastown_contract_fixture(tmp_path)
    formula = pack / "formulas" / "mol-polecat-work.toml"
    formula.write_text(
        formula.read_text(encoding="utf-8").replace(
            "if ! gc gastown polecat-submit execute; then",
            "if ! gc gastown polecat-submit execute; then\n"
    ...
    with pytest.raises(
        gascity_pack_inference_gate.GateError,
        match="terminal fence reconstructs deterministic submit behavior",
    ):
        gascity_pack_inference_gate.validate_gastown_orchestration_contract(pack)
...
@pytest.mark.parametrize(
    "required_fragment",
    gascity_pack_inference_gate.GASTOWN_POLECAT_FORMULA_COMMAND_ORDER,
)
def test_validate_gastown_orchestration_contract_rejects_missing_formula_command(
    tmp_path,
...
def test_validate_gastown_orchestration_contract_rejects_out_of_order_formula_commands(
    tmp_path,
) -> None:
    pack = write_gastown_contract_fixture(tmp_path)
    formula = pack / "formulas" / "mol-polecat-work.toml"
    first, second, *_ = gascity_pack_inference_gate.GASTOWN_POLECAT_FORMULA_COMMAND_ORDER
    text = formula.read_text(encoding="utf-8")
...
def test_gastown_build_workflow_contract_covers_orchestration_roles() -> None:
    ...
    assert "gc bd update" not in contracts["mol-polecat-work"]
    assert (
...
@pytest.mark.parametrize(
    ...
    (
        ...
        'gc workflow reopen-source "$WORK_BEAD_ID"',
...
def test_validate_gastown_orchestration_contract_rejects_split_terminal_primitive(
    tmp_path,
    fragment: str,
) -> None:
    pack = write_gastown_contract_fixture(tmp_path)
...
def test_validate_gastown_orchestration_contract_rejects_duplicate_submit_execute(
    tmp_path,
) -> None:
    pack = write_gastown_contract_fixture(tmp_path)
    formula = pack / "formulas" / "mol-polecat-work.toml"
    ...
    with pytest.raises(
        gascity_pack_inference_gate.GateError,
        match="execute exactly once",
    ):
        gascity_pack_inference_gate.validate_gastown_orchestration_contract(pack)


def test_validate_gastown_orchestration_contract_rejects_split_terminal_fence(
    tmp_path,
) -> None:
    pack = write_gastown_contract_fixture(tmp_path)
    formula = pack / "formulas" / "mol-polecat-work.toml"
    ...
    with pytest.raises(
        gascity_pack_inference_gate.GateError,
        match="exactly one shell fence",
    ):
        gascity_pack_inference_gate.validate_gastown_orchestration_contract(pack)


def test_validate_gastown_orchestration_contract_rejects_non_failing_execute(
    tmp_path,
) -> None:
    pack = write_gastown_contract_fixture(tmp_path)
    formula = pack / "formulas" / "mol-polecat-work.toml"
    ...
    with pytest.raises(
        gascity_pack_inference_gate.GateError,
        match="fail the stage",
    ):
        gascity_pack_inference_gate.validate_gastown_orchestration_contract(pack)


def test_validate_gastown_orchestration_contract_rejects_missing_execute_receipt(
    tmp_path,
) -> None:
    pack = write_gastown_contract_fixture(tmp_path)
    formula = pack / "formulas" / "mol-polecat-work.toml"
