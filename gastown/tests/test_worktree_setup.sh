#!/usr/bin/env bash
# Successor coverage for closed gastownhall/gascity-packs#368:
# polecat/refinery pre_start and worktree-setup.sh must survive paths with spaces.
set -euo pipefail
...
test_worktree_setup_restores_staged_files_after_creation_failure() {
    local tmp rig target holder branch hash
    tmp=$(mktemp -d)
    trap 'rm -rf "$tmp"' RETURN
    rig="$tmp/rig repo"
    target="$tmp/city root/.gc/worktrees/Wayfinder/polecats/gastown.nux"
...
[[ "$ran" -gt 0 ]] || fail "no tests ran"
echo "worktree-setup tests passed ($ran)"
