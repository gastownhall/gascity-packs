...
| GC-BF-004 | `implementation-base` | Virtual targeted implementation contract | Executes one implementation source anchor with prepare-worktree, implement, and close-source-anchor stages inside a durable worktree scope, then tears the worktree down through a scope/cleanup step that runs whether the scoped stages passed or failed, while preserving source-anchor close, work-item evidence, requirement coverage, and neutral producer metadata. | `implementation-base.formula.toml`; `../tests/test_formula_assets.py` |
...
| GC-BF-011 | `do-work` | Targeted implementation item helper | Extends `implementation-base`, prepares one item worktree, implements owned work with the selected implementation target, closes the source anchor after implementation succeeds, and tears the item worktree down through the inherited scope/cleanup step whether the workflow passed or failed. | `do-work.formula.toml`; `../tests/test_formula_assets.py` |
