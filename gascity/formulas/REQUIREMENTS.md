...
| GC-BF-BR-014 | GC-BF-US-002 | WHEN a build graph's `plan-review` stage is followed by decomposition, THE stage SHALL carry the `plan-review-approved.sh` check so `decompose` and the implementation drain are released only after `gc.build.plan_review_status=approved` is recorded on the workflow root, never by the `needs` edge alone. |
