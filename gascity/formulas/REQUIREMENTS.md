...
| GC-BF-BR-013 | GC-BF-US-002 | WHEN a continuation review, drain, or repair stage cannot produce review evidence, cannot apply the selected repair, or under `review_repair_policy=loop` cannot reach approval, THE formula graph SHALL record `gc.build.repair_status`, `gc.build.review_state` (`review_blocked` or `review_unavailable`), `gc.restart.*` metadata, and a failing blocked outcome instead of allowing finalize or publish no-op to close the workflow as pass. |
| GC-BF-BR-014 | GC-BF-US-002 | WHEN `review_repair_policy=once` and `review_mode` is `agent` or `interactive`, THE build-from-review suffix SHALL run exactly one `apply-review-fixes` pass that fixes every blocker and major finding, records a validated `gc.build.review-fixes.v1` artifact, `gc.build.repair_status` in {not_needed, fixed, residual}, `gc.build.review_state` in {reviewed_approved, reviewed_fixed_unverified, reviewed_with_residual_findings}, and residual finding counts by severity, and SHALL finalize as a passing candidate (`gc.build.status=candidate`) without looping; publish SHALL refuse push or PR creation while residual blocker or major findings remain. |
| GC-BF-BR-015 | GC-BF-US-001 | WHEN an extending formula binds a review expansion to the `review` step with `compose.expand`, THE expansion's terminal `{target}` step SHALL own the `gc.build.review.v1` validation gate and record `gc.build.review_report_path`, `gc.build.review_report_sha256`, `gc.build.review_verdict`, and `gc.build.review_initial_findings`, and the base `review` step SHALL carry neither an inline `expand` nor a default `compose.expand` rule. |
...
| GC-BF-007 | `fix-loop-base` | Virtual targetless review-fix contract | Turns failed review findings into planned fixes, applies fixes with the selected implementation path, records per-attempt validated fix artifacts, and re-runs the selected review formula up to the iteration limit. One iteration is one fix pass followed by one fresh re-review; `build-from-review-base` dispatches this loop only under `review_repair_policy=loop`. | `fix-loop-base.formula.toml`; `../tests/test_formula_assets.py` |
...
| GC-BF-009 | `build-basic-review` | Expansion review loop | Runs starter review fanout across acceptance/correctness, test evidence, and simplicity/maintainability lanes, provides one override file per lane, synthesizes findings, applies required fixes, validates review/fix artifacts, and loops until approved, blocked, or attempts exhaust. Its terminal `{target}` step satisfies the `build-from-review-base` review slot contract (GC-BF-BR-015). | `build-basic-review.formula.toml`; `../tests/test_formula_assets.py` |

### Continuation Entrypoints

| ID | Formula | Type | Required behavior | Evidence |
| --- | --- | --- | --- | --- |
| GC-BF-025 | `build-from-review-base` | Virtual targetless review suffix | Validates implementation evidence, exposes `review` as a compose-expansion slot, runs the selected code-review, applies review findings once (`review_repair_policy=once`) or repairs through the bounded `review_fix_formula` loop (`loop`), records the post-fix review state and residual findings, finalizes candidates with residual findings only under `once`, never converts blocked or unavailable review states to pass, and optionally publishes. Higher suffixes hand off to this base through `prepare-review`. | `build-from-review-base.formula.toml`; `../tests/test_formula_assets.py::FormulaAssetTests::test_build_continuation_bases_form_nested_suffix_chain`; `../tests/test_formula_assets.py::FormulaAssetTests::test_build_from_review_blocked_results_are_healable_not_passed`; `../tests/test_formula_assets.py::FormulaAssetTests::test_producer_stages_gate_artifacts_with_bounded_repair`; `../tests/test_formula_assets.py::FormulaAssetTests::test_build_from_review_review_step_is_a_compose_expansion_slot`; `../tests/test_formula_assets.py::FormulaAssetTests::test_build_from_review_once_policy_is_a_single_pass_candidate`; `../tests/test_formula_assets.py::FormulaAssetTests::test_review_fixes_schema_validates_sample_artifacts` |
