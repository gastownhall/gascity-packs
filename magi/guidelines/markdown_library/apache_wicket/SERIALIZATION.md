...
Mark non-serializable fields as `transient` and reinitialize on access. With Spring integration, use `@SpringBean` for automatic handling — Spring beans inject as serializable proxies. Without DI, implement lazy initialization checking for null before use:

```java
public class ReportPanel extends Panel {

    private transient ExpensiveCalculator calculator;

    private ExpensiveCalculator getCalculator() {
        if (calculator == null) {
            calculator = new ExpensiveCalculator();
        }
        return calculator;
    }
}
