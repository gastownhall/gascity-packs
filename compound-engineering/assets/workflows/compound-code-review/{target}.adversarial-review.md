Run the installed Compound Engineering adversarial review persona.

This bead only runs after the cheap conditional gate selected adversarial
review for this change. Construct concrete failure scenarios for the selected
large or high-risk diff and return structured findings for synthesis. Do not
re-run applicability as a no-op; skipped adversarial lanes are closed by the
gate.
...
`<code-review-artifact-root>/adversarial.md`. Record that exact path on the
