...
QUESTION_VERSION = 'gc.evidence-question.v1'
CHOICES = {
    ...
    'contradicted': 'The supplied source or actual proof explicitly contradicts the claim. Inspect implementation and test before choosing a repair.',
...
def evaluate(state: dict, *, model: str, timeout: float = 30, record_dir: Path | None = None) -> dict:
    key = os.environ.get('TYPESAFE_API_KEY')
    if not key:
        raise ValueError('TYPESAFE_API_KEY is required for live Jev evaluation')
    payload = {'model': model, 'state': state, 'questions': questions(state)}
    if record_dir:
