...
def main(argv=None):
 ...
 report={'schema':'gc.duplicate-ranking.v1','question_version':QUESTION_VERSION,'route':'ordinary_investigation','usage':None,'started_at':datetime.now(timezone.utc).isoformat()}
 try:
  ...
  if a.mode=='off' or (a.mode=='auto' and not os.environ.get('TYPESAFE_API_KEY')):
   ...
   response=transport.evaluate(state,model=a.model,question_set=questions) if questions else {'model':a.model,'answers':{},'usage':{'input_tokens':0,'output_tokens':0}}
