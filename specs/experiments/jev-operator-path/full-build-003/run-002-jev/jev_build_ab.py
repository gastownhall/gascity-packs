...
def stop_disposable_dolt(city):
    """Stop leftover servers only when their exact paths are inside this run's city.

    Gas City 1.4 runs Dolt from the dolt pack's runtime directory. Gas City 1.5
    runs it under `.beads/dolt` behind a `bd db-proxy-child` that `gc stop`
    leaves running; the proxy is stopped before its server.
    """
    city = city.resolve()
    configs = {city/'.gc/runtime/packs/dolt/dolt-config.yaml', city/'.beads/dolt/config.yaml'}
    proxy_root = city/'.beads/dolt'
    ...
    for line in result.stdout.splitlines():
        ...
        if proxy and Path(proxy.group(1)).resolve() == proxy_root:
