...
follows is the standard `pr-pipeline` applies to a fix in the dolt / beads /
gascity / packs stack, and the layer map you need to know which of those four
repositories a defect actually belongs to.
...
real `gc` against the shipped pack directory, not a copy of it. See
`tests/test_pack_composition_warnings.py` and
`tests/test_gc_role_prompt_integration.py` in this repository for two working
harnesses.
...
the one that matters. Perturb each stub toward a plausible alternative real
...
lint loop and `registry.toml`: **11 of the 16 registry packs are never exercised
against a running `gc`**. Those eleven are `cass`, `contributing`, `discord`,
`gastown`, `github`, `oversight-rig`, `pr-pipeline`, `runtime-cloudflare`,
`slack-channel`, `slack-full` and `slack-mini`. The five covered are `bmad`,
`compound-engineering`, `gascity`, `gstack` and `superpowers`. Counting the lint
loop's entries overstates coverage by one, because `profiler` is linted and is
not a registry pack. Issue **#307** reports this at ten.
...
Every number above is re-derivable. `$GASCITY` is a checkout of
`gastownhall/gascity`, `$PACKS` a checkout of `gastownhall/gascity-packs`.
...
# registry packs, and which of them that loop covers
python3 -c 'import tomllib;print(sorted(p["name"] for p in tomllib.load(open("registry.toml","rb"))["pack"]))'
