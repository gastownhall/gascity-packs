...
record changed files, tests run, resolved findings, and blockers. If the
synthesized report approves the implementation with no required fixes and this
lane made no mutation, record a no-op review-fix artifact, update workflow root
